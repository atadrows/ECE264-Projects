...
double mid_point_numerical_integration(double lower_limit, double upper_limit, int n_intervals)
{
	...
	double midpoint_width = width / 2;

	for (int i = 0; i < n_intervals; i++){
		double midpoint_x = lower_limit + i * width + midpoint_width;
		double rect_area = function_to_be_integrated(midpoint_x) * width;
...
double trapezoidal_numerical_integration(double lower_limit, double upper_limit, int n_intervals)
{
   double integral = 0.0;
	...
	for (int i = 0; i < n_intervals; i++){
		double trap_left = lower_limit + i * width;
		double trap_area = width * (function_to_be_integrated(trap_left) + function_to_be_integrated(trap_left + width)) / 2;
